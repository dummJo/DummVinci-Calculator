...
export default function GamePage() {
  ...
  // Local "shake" animation key per tile to re-trigger CSS when a merge happens
  const [, setTick] = useState(0);
  const prevGrid = useRef(state.grid);
  useEffect(() => {
    if (!gridsEqual(prevGrid.current, state.grid)) {
      setTick(t => t + 1);
    }
    prevGrid.current = state.grid;
  }, [state.grid]);
  ...
  return (
    <>
      <style>{`
        ...
        .score-box {
          background: var(--card-bg, rgba(var(--accent-rgb),0.08));
      ...
      <main className="game-page" data-no-ptr>
        {/* Header */}
        ...
        <div className="game-footer">
          PTTS Praxis — Break Room · By DummVinci
        </div>
      </main>
    </>
  );
